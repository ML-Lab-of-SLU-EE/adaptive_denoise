...
class MappingNetwork(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        for i in range(3):
            ...
            layers += [Relu(name='shared_relu_' + str(i))]
        ...
        for n_d in range(self.num_domains):
            for i in range(3):
                ...
                layers += [Relu(name='domian_{}_relu{}'.format(n_d, i))]
...
class StyleEncoder(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        blocks += [Leaky_Relu(alpha=0.2, name='leaky_0')]
        blocks += [Conv(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv')]
        blocks += [Leaky_Relu(alpha=0.2, name='leaky_1')]
...
class Discriminator(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        blocks += [Leaky_Relu(alpha=0.2, name='dis_leak_0')]
        blocks += [Conv(channels=ch_out, kernel=4, stride=1, pad=0, sn=self.sn, name='conv_0')]

        blocks += [Leaky_Relu(alpha=0.2, name='dis_leak_1')]
