...
def _check(images, dtypes, min_value=-np.inf, max_value=np.inf):
    """check if images in range [min_value, max_value]"""
    # check type
...
def to_range(images, min_value=0.0, max_value=1.0, dtype=None):
    ...
    _check(images, [np.float32, np.float64], -1.0, 1.0)  # TODO:只能保存np.float32和np.float64的数据类型作为图片
